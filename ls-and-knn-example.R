...
library('caret')
knn_fit <- knnreg(Rating ~ Education + Income, k=2, data=Credit)
...
ls_fun <- function(x1, x2, fit=ls_fit) 
  predict(fit, data.frame(Education=x1, Income=x2))
plot_credit_data(ls_fun)
train_err_int <- mean((Credit$Rating - predict(ls_fit, Credit))^2)

## compare additive error versus interaction error model
## training error of additive model is about 0.4% bigger than
## that of interaction error model
...
library('caret')
knn_fit <- knnreg(Rating ~ Education + Income, k=5, data=Credit)
knn_fun <- function(x1, x2, fit=knn_fit) 
  predict(fit, data.frame(Education=x1, Income=x2))
plot_credit_data(knn_fun)
mean((Credit$Rating - predict(knn_fit, Credit))^2)
summary(cvknnreg(kNN=5, flds=inc_flds))
